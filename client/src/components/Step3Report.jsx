...
function Step3Report({ report }) {
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-gray-50 to-purple-50 px-4 sm:px-6 lg:px-10 py-8'>
      <div className='mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
        ...
        <button onClick={downloadPDF} className='bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-xl shadow-md transition-all duration-300 font-semibold text-sm sm:text-base text-nowrap'>Download PDF</button>
      </div>
                ...
                skills.map((s, i) => (
                  <div key={i}>
                    <div className='flex justify-between mb-2 text-sm sm:text-base'>
                      ...
                      <span>{s.label}</span>
                      <span className='font-semibold text-purple-600'>{s.value}</span>
                    </div>
              ...
              {questionWiseScore.map((q, i) => (
                <div key={i} className='bg-gray-50 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200'>
                  ...
                  <div className='flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-4'>
                    ...
                    <div className='bg-purple-100 text-purple-600 px-3 py-1 rounded-full font-bold text-xs sm:text-sm w-fit'>
                      {q.score ?? 0}/10
                    </div>
                  </div>
